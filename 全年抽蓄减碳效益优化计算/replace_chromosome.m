...
% added one by one until addition of a complete front which results in该功能根据等级和拥挤距离来替换染色体。
% exceeding the population size. At this point the chromosomes in that最初，直到人口规模达到每个前沿逐一加入，直到增加一个完整的前面，导致超过人口规模。 
% front is added subsequently to the population based on crowding distance此时，根据拥挤距离，将该前沿的染色体随后添加到群体中。. 
