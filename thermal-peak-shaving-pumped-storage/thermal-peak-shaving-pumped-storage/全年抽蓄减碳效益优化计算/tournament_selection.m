...
% |pool_size|. Selection is performed based on two criteria（标准）. First and
% foremost（根本） is the rank or the front（等级或前沿） in which the solutions reside.
...
% while the number of elements in chromosome are important.染色体中元素的数量是重要的
[pop, variables] = size(chromosome);
% The peunltimate（倒数第二） element contains the information about rank.
rank = variables - 1;
...
for i = 1 : pool_size
    ...
    for j = 1 : tour_size
        ...
        % Make sure that the array（排列） starts from one. 
        if candidate(j) == 0
            candidate(j) = 1;
        end
        if j > 1
    ...
    % Collect information about the selected candidates（候选）.
    for j = 1 : tour_size
        c_obj_rank(j) = chromosome(candidate(j),rank);
        c_obj_distance(j) = chromosome(candidate(j),distance);
    end
    % Find the candidate with the least rank 找出排名最低的
    min_candidate = ...
    ...
    if length(min_candidate) ~= 1  %length:即数列的长度
        max_candidate = ...
